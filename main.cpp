...
void initDatabase()
{
    try
    {
        ...
        {
            cout << "[DB] Database opened successfully" << endl;
        }
    }
...
public:
    ...
    bool operator>(user_account student) const
...
public:
    university(string _name, string _country, string _desc, string _contact, unsigned short _prog,
               ...
               double _tution_fees, float _gpa, float _duration, int _ranking) : name(_name), country(_country), desc(_desc), contact(_contact), programme(_prog), age(_age),
...
{
    if (student.getGPA() >= uni.getGPA() && student.getEnglishLvl() >= uni.getEnglishLvl() && student.getDegreeLvl() == uni.getDegreeLvl())
        return true;
    else
        return false;
}
...
inline void bankDepositWithdrawDisplay(string accountName, double balance, string amount, bool type, unsigned short msg)
{
    ...
    cout << "|=-=-=-=-=|    BankingSystem    |=-=-=-=-=|\n"
         ...
         << (type ? ((msg == 1) ? "|[BANK]| Transaction Failed, amount to deposit must be at least 5$.\n\n" : (msg == 2) ? "|[BANK]| Transaction Failed, amount to deposit must NOT exceed 10,000$.\n\n"
...
void bankMainMenu(bank_account &account, bool showInfo = false)
{
    ...
    case 1:
        ...
        break;
    case 2:
        bankWithdraw(account);
        break;
    case 3:
        bankMainMenu(account);
        break;
    case 4:
        bankMainMenu(account, true);
        break;
    case 5:
        bankChangePIN(account);
        break;
    case 0:
        bank();
        return;
...
void studyFinderCustomSearch(user_account &account, string country = "___", string deg_lvl = "___", string stu_prog = "___", string gpa = "___", string eng_lvl = "___", string tuition = "___")
{
    ...
    break;
    case 0:
        studyFinderSearchEngine(account);
        return;
    }
}
...
void studyFinderSearchEngine(user_account &account)
{
    ...
    {
    case 1:
        studyFinderCustomSearch(account);
        break;
    case 2:
        studyFinderSearchBasedOnProfile(account);
        break;
    case 3:
        studyFinderShowAllOptions(account);
        break;
